...
<h1 class="note-form-title mb-4">Edit Note</h1>
...
<form method="POST" action="/<?= $note['id'] ?>/edit">
  <?= csrf_field() ?>

  <div class="mb-3">
    <label for="noteTitle" class="form-label">Title</label>
    <input type="text" id="noteTitle" name="title" class="form-control" placeholder="Give your note a title…" required maxlength="255" value="<?= esc($note['title']) ?>" />
  </div>
  ...
  <div class="mb-4">
    ...
    <input type="hidden" name="content" id="noteContent" value="<?= esc($note['content'] ?? '') ?>" />
  </div>
